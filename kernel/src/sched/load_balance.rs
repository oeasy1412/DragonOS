...
impl LoadBalancer {
    ...
    pub fn select_task_rq(
        ...
        if wake_flags & WakeupFlags::WF_TTWU.bits() != 0 {
        ...
        if wake_flags & WakeupFlags::WF_FORK.bits() != 0 {
            let target = Self::find_idlest_cpu_lockless(cpus_allowed, prev_cpu);
            return Self::fallback_if_not_allowed(target, prev_cpu, cpus_allowed);
        }
    ...
    fn select_idle_sibling(
        ...
        if cpus_allowed.get(target).unwrap_or(false) && is_idle_cpu(target) {
            return target;
        }
        if prev_cpu != ProcessorId::INVALID
            && prev_cpu != target
            && cpus_allowed.get(prev_cpu).unwrap_or(false)
            && is_idle_cpu(prev_cpu)
        {
            return prev_cpu;
        }
...
pub fn update_sg_lb_stats(sg: &SchedGroup, sgs: &mut SgLbStats, env: &LbEnv) {
    *sgs = SgLbStats::default();
    ...
    for cpu in (&sg.cpumask & &env.cpus).iter_cpu() {
        ...
        if nr_running == 0 && is_idle_cpu(cpu) {
            sgs.idle_cpus += 1;
        }
    }
    ...
    if sgs.group_type == GroupType::Overloaded {
        sgs.avg_load = sgs
            .group_load
            .checked_mul(SCHED_CAPACITY_SCALE)
            .and_then(|v| v.checked_div(sgs.group_capacity))
            .unwrap_or(0);
    }
}
            ...
            && !env.flags.contains(LbfFlags::ACTIVE_LB)
        {
            ...
            if let Some(sd) = sd {
                if let Some(ref group) = sd.groups {
                    let candidates = &group.cpumask & &env.cpus;
                    for cpu in candidates.iter_cpu() {
                        if cpus_allowed.get(cpu).unwrap_or(false) {
...
/// 判断是否需要主动均衡
/// 当常规 detach_tasks 无法拉取任务时，检查是否应该推送 busiest CPU 的当前运行任务到 dst_cpu
fn need_active_balance(env: &LbEnv) -> bool {
    // 条件 1: imbalanced_active_balance
    let cache_nice_tries: u32 = 1;
    let nr_failed = env
        .sd
        .as_ref()
        .map(|sd| sd.nr_balance_failed.load(Ordering::Relaxed))
        .unwrap_or(0);
    if nr_failed > cache_nice_tries + 2 {
        return true;
    }

    // 条件 2: dst CPU 空闲且 src 只有 1 个 CFS 任务
    if env.idle != super::rebalance::CpuIdleType::NotIdle {
        let src_rq = cpu_rq(env.src_cpu.data() as usize);
        if src_rq.cfs_h_nr_running_lockless() == 1 {
            return true;
        }
    }
...
pub fn load_balance(
    ...
    while redo {
        ...
        // cpu_is_online(busiest_cpu) 检查
        if !smp_cpu_manager().is_online_cpu(busiest_cpu) {
