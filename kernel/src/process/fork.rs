...
use crate::{
    ...
    filesystem::{
        ...
        vfs::{
            file::{File, FileFlags, ReservedFd},
            FileType,
        },
    },
        ...
        if !clone_flags.contains(CloneFlags::CLONE_UNTRACED) {
                ...
                || current_pcb.ptrace_event_enabled(event)
            {
                ...
                if let Some(tracer) = ptracer_of(current_pcb) {
                    // ptrace_link 成功后，后续步骤失败必须 ptrace_unlink 兜底
                    pcb.ptrace_link(&tracer)?;
                    ...
                    if is_seized {
                        ...
                        pcb.ptrace_state.lock_irqsave().pending_event_stop = Some(Signal::SIGTRAP);
