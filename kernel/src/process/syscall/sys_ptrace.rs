...
impl Syscall for SysPtrace {
    ...
    fn handle(&self, args: &[usize], _frame: &mut TrapFrame) -> Result<usize, SystemError> {
        ...
        // 查找目标进程。
        let tracee = ProcessManager::find(pid).ok_or(SystemError::ESRCH)?;
        ...
        let result: isize = match request {
            ...
            PtraceRequest::Getsyscallinfo => {
                ...
                let actual: usize = match info.op {
                    ptrace::PtraceSyscallInfoOp::None => 24,
                    ptrace::PtraceSyscallInfoOp::Exit => 40,
                    ptrace::PtraceSyscallInfoOp::Entry => 80,
                    ptrace::PtraceSyscallInfoOp::Seccomp => {
                        core::mem::size_of::<ptrace::PtraceSyscallInfo>()
                    }
                };
                // 截断到用户缓冲区大小，避免溢出。
